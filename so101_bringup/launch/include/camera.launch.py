...
def parse_cameras_config(
    ...
    for cam in cfg["cameras"]:
        ...
        if not name or not cam_type or not rel_param:
            raise ValueError(
                f"Camera entry is missing one of 'name'/'type'/'param_path': {cam}"
            )
        if cam_type not in TYPE_REGISTRY:
            raise ValueError(
                f"Unknown camera type '{cam_type}'. "
                f"Supported: {list(TYPE_REGISTRY.keys())}"
            )
...
def build_node_for_camera(spec: CameraSpec) -> Node:
    pkg, exe = TYPE_REGISTRY[spec.type]
    ...
    if remappings is None and spec.type == "usb_camera":
