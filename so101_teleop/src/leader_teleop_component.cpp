...
: rclcpp::Node("leader_teleop_component", options)
{
  ...
  auto leader_topic = this->declare_parameter<std::string>(
    ...
    "/leader/joint_states");
  auto follower_topic = this->declare_parameter<std::string>(
  ...
  RCLCPP_INFO(
    ...
    leader_topic.c_str());
  RCLCPP_INFO(
    this->get_logger(), "Publishing to follower trajectory on: '%s'",
    follower_topic.c_str());

  // Create publisher and subscriber
  trajectory_pub_ =
    this->create_publisher<trajectory_msgs::msg::JointTrajectory>(follower_topic, 10);
...
void LeaderTeleopComponent::joint_state_callback(const sensor_msgs::msg::JointState::SharedPtr msg)
{
  ...
  if (!is_initialized_) {
    ...
    RCLCPP_INFO(
      this->get_logger(), "Initialized with joint order: [%s, ...]",
      ordered_joint_names_[0].c_str());
  }
  ...
  // print the point for debugging with RCLCPP_INFO
  // RCLCPP_INFO(this->get_logger(), "Publishing trajectory point with positions", point.positions);
