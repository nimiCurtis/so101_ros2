...
void JoyToServoComponent::JoyCBLoop(const sensor_msgs::msg::Joy::ConstSharedPtr & msg)
{
  ...
  if (convertJoyToCmd(msg->axes, msg->buttons, twist_msg, joint_msg)) {
    RCLCPP_INFO(
      this->get_logger(), "Publishing twist command: lin[x,y,z]=[%.2f, %.2f, %.2f], ang[x,y,z]=[%.2f, %.2f, %.2f]",
    ...
    RCLCPP_INFO(
      this->get_logger(), "Publishing joint velocity command with %zu velocities in frame: %s",
      joint_msg->velocities.size(), frame_to_publish_.c_str());
