...
class SmolVLAInferenceNode(Node):
    def __init__(self):
        ...
        self.get_logger().info('PI0.5 Inference Node starting...')
        ...
        # CRITICAL: Also convert buffers explicitly
        ...
        # # Disable compilation for inference (compiled models can be unstable)
        # if hasattr(self.model.config, 'compile_model'):
        #     self.model.config.compile_model = False
        #     self.get_logger().info('Disabled model compilation for inference')
        ...
        # After loading model and setting to eval mode, add:
        ...
        self.pre_processor, self.post_processor = make_pre_post_processors(
            ...
            preprocessor_overrides={
                "device_processor": {
                    ...
                    "float_dtype": "bfloat16"}}, # fit compile dtype "bfloat16"
        )
    ...
    def inference_callback(self):
        ...
        try:
            ...
            self.get_logger().info('=== DEBUG: Keys after preprocessing ===')
            for key in sorted(obs.keys()):
                if isinstance(obs[key], torch.Tensor):
                    self.get_logger().info(f'{key}: {obs[key].shape}')
            ...
            self.get_logger().info('🔄 Starting model inference...', throttle_duration_sec=2.0)
            ...
            try:
                    ...
                    with torch.amp.autocast(device_type='cuda',dtype=torch.bfloat16):
                        ...
                        for key, value in obs.items():
                            if isinstance(value, torch.Tensor):
                                ...
                                self.get_logger().info(f'Input key: {key}, dtype: {original_dtype}, shape: {value.shape}', throttle_duration_sec=10.0)
                                # Tokens must be long (int64)
                                if 'tokens' in key:
                                    if value.dtype != torch.long:
                                        obs[key] = value.to(dtype=torch.long)
                                
                                # Attention masks MUST be bool
                                elif 'attention_mask' in key:
                                    if value.dtype != torch.bool:
                                        obs[key] = value.to(dtype=torch.bool)
                                ...
                                # Everything else that's float should match model dtype
                                    ...
                                    if value.dtype != model_dtype:
                                        obs[key] = value.to(dtype=model_dtype)
                                        if original_dtype != model_dtype:
                                            self.get_logger().info(
                                                f'Converted {key} from {original_dtype} to {model_dtype}',
                ...
                return  # Exit early if inference fails
